    ...
    <section id="skills" className="py-20 relative">
      ...
      <div className="container mx-auto px-6 md:px-12 relative z-10">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-end justify-between mb-12">
          ...
          <div className="font-mono text-xs text-gray-600 mt-2 md:mt-0 flex items-center gap-2">
        ...
        >
          {SKILL_CATEGORIES[activeCategory].items.map((skill, i) => (
            <MotionDiv
              ...
              className="glass-dark rounded-2xl p-6 flex flex-col items-center text-center hover:border-primary/20 transition-all duration-300 group cursor-default"
            >
              <div className="relative mb-3">
                <ProgressRing progress={skill.level} />
                <span className="absolute inset-0 flex items-center justify-center text-lg font-bold font-display text-white group-hover:text-primary transition-colors">
                  {skill.level}
                </span>
              </div>
              <span className="text-white font-bold text-sm font-display">{skill.name}</span>
              <span className="text-[10px] font-mono text-gray-600 mt-1">PROFICIENCY</span>
            </MotionDiv>
          ))}
        </motion.div>
      </div>
